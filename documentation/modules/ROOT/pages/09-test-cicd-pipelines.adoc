...
cd gramola-events
sed -i 's/info.name=.*/info.name=gramola-events.{username}-'${RANDOM}'/g' ./src/main/resources/application.properties
git commit ./src/main/resources/application.properties -m "update info.name" && git push
cd ..
rm -rf gramola-events
----
...
Use this link to watch the pipeline running.

[.console-input]
[source,bash, subs="+attributes"]
----
https://console-openshift-console.apps.%BASE_SUBDOMAIN%/dev-pipelines/ns/gramola-cicd-{username}
----
