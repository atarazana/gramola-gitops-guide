...
----
https://{argocd-server}/applications&labels=argocd-cicd-app
----
...
Additionally we have to create the same secret in `dev` and `test` and link it to the default `ServiceAccount` for pulling.
